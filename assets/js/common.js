/*
 * Vistaraz — Common UI (nav + dark mode + i18n + crisis + SOS + tour + a11y)
 */
(function (global) {
  "use strict";
  ...
  function applyTheme() {
    let t = "light";
    try { t = localStorage.getItem("vistaraz_theme") || "light"; } catch (e) {}
  ...
  function toggleTheme() {
    ...
    try { localStorage.setItem("vistaraz_theme", next); } catch (e) {}
    const btn = document.getElementById("themeBtn");
  ...
  function toggleContrast() {
    ...
    document.documentElement.setAttribute("data-theme", next);
    Vistaraz.common.toast(next === "contrast" ? "High contrast on" : "High contrast off");
  }

  function applyLang() {
    const l = global.Vistaraz.i18n.getLang();
    const sel = document.getElementById("langSel");
    if (sel) sel.value = l;
    document.querySelectorAll("[data-i18n]").forEach(el => {
      el.textContent = global.Vistaraz.i18n.t(el.getAttribute("data-i18n"));
    });
  }
  ...
  function navHtml() {
    ...
    <nav class="bottom-nav" aria-label="Primary">
      ${NAV.map(([href, key, ico]) =>
        `<a href="${href}" class="${href === currentPage() ? "active" : ""}"><span class="bi">${ico}</span>${global.Vistaraz.i18n.t(key)}</a>`
      ).join("")}
    </nav>`;
  }
  ...
  function tourHtml() {
    ...
    <div id="tour-overlay">
      <div class="tour-card">
        ...
        <h2 id="tourTitle" style="margin:0 0 8px">Welcome to Vistaraz</h2>
  ...
  function endTour(){
    ...
    try { localStorage.setItem("vistaraz_toured","1"); } catch(e){}
  }
  function maybeTour(){
    let seen = "0";
    try { seen = localStorage.getItem("vistaraz_toured") || "0"; } catch(e){}
    if(seen !== "1"){
      const o = document.getElementById("tour-overlay");
  ...
  function mount() {
    ...
    // inline handlers referencing Vistaraz.common.* fail on some mobile browsers).
    ...
    if (sel) sel.addEventListener("change", e => {
      global.Vistaraz.i18n.setLang(e.target.value); applyLang();
      global.Vistaraz.common.toast("Language: " + e.target.value.toUpperCase());
    });

    try {
      const sev = global.Vistaraz.state.get().lastSeverity;
  ...
  function pwaInit() {
    ...
    window.addEventListener("offline", () => toast("📴 You're offline — Vistaraz still works"));
    ...
    window.addEventListener("appinstalled", () => {
      ...
      toast("✅ Vistaraz installed — thanks!");
    });
  }
